...
class TestBMaxSumMetric:
    def test_identical_plans_score_zero(self, task, plan_l1_then_l2):
        counter = BehaviourDiversityCounter(
            task, [plan_l1_then_l2, plan_l1_then_l2], [('go', None)]
        )

        assert counter.compute_b_maxsum_metric() == 0.0
    ...
    def test_fully_reordered_plans_score_one(
        ...
        assert counter.compute_b_maxsum_metric() == 1.0
    ...
    def test_score_averages_over_dimensions(
        ...
        assert counter.compute_b_maxsum_metric() == 0.25
    ...
    def test_score_averages_over_all_plan_pairs(
        ...
        assert counter.compute_b_maxsum_metric() == pytest.approx(2 / 3)
    ...
    def test_b_maxsum_works_with_the_cost_bound_dimension(
        ...
        objects while compute_b_maxsum_metric passes behaviour strings.
        ...
        assert counter.compute_b_maxsum_metric() == 0.5
    ...
    def test_dimensions_without_a_distance_cannot_be_scored(
        ...
        defined over the dimensions that implement one."""
        counter = BehaviourDiversityCounter(
            task, [plan_l1_then_l2, plan_l2_then_l1], [('rc', resource_file)]
        )

        with pytest.raises(AssertionError):
            counter.compute_b_maxsum_metric()
