...
string, then bdc / extract / b_maxsum over those strings."""
...
class TestConstruction:
    ...
    def test_plan_list_is_materialised_from_any_iterable(self, task, plan_l1_then_l2):
        ...
        assert counter.plans == [plan_l1_then_l2]
...
class TestBDC:
    ...
    def test_bdc_is_idempotent(self, task, plan_l1_then_l2, plan_l2_then_l1):
        ...
        assert counter.bdc() == counter.bdc() == 2
...
class TestExtract:
    def test_returns_at_most_k_plans(self, task, plan_l1_then_l2, plan_l2_then_l1):
        ...
        assert len(counter.extract(k=1)) == 1
    ...
    def test_returns_every_plan_when_k_exceeds_the_pool(
        ...
        assert len(counter.extract(k=99)) == 2
    ...
    def test_k_of_zero_selects_nothing(self, task, plan_l1_then_l2):
        ...
        assert counter.extract(k=0) == []

    def test_extract_populates_behaviours(
        self, task, plan_l1_then_l2, plan_l2_then_l1
    ):
        counter = BehaviourDiversityCounter(
            task, [plan_l1_then_l2, plan_l2_then_l1], [('go', None)]
        ...
        assert len(counter.behaviours) == 2
    ...
    def test_unknown_indicator_is_rejected(self, task, plan_l1_then_l2):
        ...
        with pytest.raises(ValueError, match='valid indicators'):
            counter.extract(k=1, indicator='nope')


class TestExtractBMaxSum:
    """extract(k, indicator='bmaxsum') -- the greedy extraction: repeatedly add
    the plan whose behaviour has the greatest summed distance to the behaviours
    already selected."""
    ...
    def test_greedy_covers_distinct_behaviours_before_duplicates(
        ...
        selected = counter.extract(k=2, indicator='bmaxsum')
    ...
    def test_greedy_adds_the_farthest_behaviour_first(self, task, domain, make_plan):
        ...
        counter = BehaviourDiversityCounter(
            task, [cost2, cost3, cost4], [('cb', {'q': 2.0})]
        )
    ...
    def test_duplicates_are_picked_only_when_the_pool_is_exhausted(
        ...
        selected = counter.extract(k=3, indicator='bmaxsum')
...
class TestBMaxSum:
    ...
    def test_the_metric_sums_over_distinct_behaviour_pairs(
        ...
        assert counter.b_maxsum() == pytest.approx(13 / 12)
    ...
    def test_dimensions_without_a_distance_cannot_be_scored(
        ...
        counter = BehaviourDiversityCounter(
            task, [plan_l1_then_l2, plan_two_trucks], [('rc', resource_file)]
        )
